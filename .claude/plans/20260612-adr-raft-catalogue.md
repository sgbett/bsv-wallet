...
reference doc the living statement (the ADR-001 split). `schema-intent.md` is
...
- **Normative living principle — reference home *plus* ADR (the ADR-001 split):** `principle-of-state.md` (↔ ADR-001), `state-boundaries.md` (↔ G5).
...
| G5 | **Stateless-SDK / stateful-wallet boundary** | KEEP | ADR records the boundary *decision*; `state-boundaries.md` stays as the living statement (ADR-001 split). brc100-impl #5 (drop ARC facade); HLR #302 |
