    ...
    module Postgres
      module Store
        class Output < Sequel::Model
          ...
          one_to_one   :spendable_entry, class: 'BSV::Wallet::Postgres::Store::Spendable', key: :output_id
          one_to_one   :detail, class: 'BSV::Wallet::Postgres::Store::OutputDetail', key: :output_id
          one_to_one   :input, class: 'BSV::Wallet::Postgres::Store::Input', key: :output_id
          one_to_one   :output_basket, class: 'BSV::Wallet::Postgres::Store::OutputBasket', key: :output_id
          many_to_many :tags, class: 'BSV::Wallet::Postgres::Store::Tag',
          ...
          dataset_module do
            ...
            def spendable
              spendable_ds = BSV::Wallet::Postgres::Store::Spendable.dataset
                .where(output_id: Sequel[:outputs][:id]).select(1)
              input_ds = BSV::Wallet::Postgres::Store::Input.dataset
                .where(output_id: Sequel[:outputs][:id]).select(1)
            ...
            def in_basket(name)
              if name == 'default'
                basket_ds = BSV::Wallet::Postgres::Store::OutputBasket.dataset
