...
RSpec.describe BSV::Wallet::Postgres::Action do
  ...
  describe 'creation' do
    ...
    it 'preserves binary wtxid' do
      ...
      action = described_class.create(outgoing: true, description: 'test action', wtxid: wtxid, raw_tx: raw_tx)
    ...
    it 'rejects corrupt wtxid (hex stored as binary) at database level' do
      ...
      expect {
        described_class.create(outgoing: true, description: 'test action', wtxid: Sequel.blob(hex_value), raw_tx: raw_tx)
  ...
  describe 'associations' do
    it 'belongs to tx_proof' do
      action = described_class.create(outgoing: true, description: 'test action', wtxid: SecureRandom.random_bytes(32), raw_tx: raw_tx, tx_proof_id: tx_proof.id)
  ...
  describe '#derived_status' do
    ...
    it 'returns :nosend when broadcast is none' do
      action = described_class.create(outgoing: true, description: 'test action', wtxid: SecureRandom.random_bytes(32), raw_tx: raw_tx, broadcast: 'none')
