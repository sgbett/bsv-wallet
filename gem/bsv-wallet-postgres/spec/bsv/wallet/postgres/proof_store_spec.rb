...
RSpec.describe BSV::Wallet::Postgres::ProofStore do
  ...
  let(:txid) { SecureRandom.random_bytes(32) }
  let(:proof_data) do
    {
      height: 800_000,
      block_index: 42,
  ...
  describe '#save_proof' do
    it 'creates a new proof' do
      id = proof_store.save_proof(txid: txid, proof: proof_data)
      expect(id).to be_a(Integer)
    end

    it 'upserts an existing proof' do
      id1 = proof_store.save_proof(txid: txid, proof: proof_data)
      id2 = proof_store.save_proof(txid: txid, proof: proof_data.merge(height: 800_001))
    ...
    it 'preserves binary data' do
      proof_store.save_proof(txid: txid, proof: proof_data)
      record = BSV::Wallet::Postgres::TxProof.first(txid: Sequel.blob(txid))

      expect(record.txid.encoding).to eq(Encoding::BINARY)
  ...
  describe '#find_proof' do
    it 'returns the proof hash' do
      proof_store.save_proof(txid: txid, proof: proof_data)
      result = proof_store.find_proof(txid: txid)
    ...
    it 'returns nil when not found' do
      expect(proof_store.find_proof(txid: SecureRandom.random_bytes(32))).to be_nil
    end
  end
  ...
  describe '#proof_exists?' do
    it 'returns true when proof exists' do
      proof_store.save_proof(txid: txid, proof: proof_data)
      expect(proof_store.proof_exists?(txid: txid)).to be true
    end

    it 'returns false when no proof' do
      expect(proof_store.proof_exists?(txid: SecureRandom.random_bytes(32))).to be false
    end
  end

  describe '#request_proof' do
    it 'creates a tx_req entry' do
      proof_store.request_proof(txid: txid)
      req = BSV::Wallet::Postgres::TxReq.first(txid: Sequel.blob(txid))
    ...
    it 'is idempotent — duplicate txid is ignored' do
      proof_store.request_proof(txid: txid)
      proof_store.request_proof(txid: txid)

      expect(BSV::Wallet::Postgres::TxReq.where(txid: Sequel.blob(txid)).count).to eq(1)
    end

    it 'stores raw_tx and input_beef as binary' do
      raw_tx = SecureRandom.random_bytes(200)
      beef = SecureRandom.random_bytes(100)
      proof_store.request_proof(txid: txid, raw_tx: raw_tx, input_beef: beef)
  ...
  describe '#process_pending' do
    ...
    context 'with arc_client' do
      ...
      it 'resolves pending requests and creates proofs' do
        ...
        expect(results.first[:txid]).to eq(txid)
