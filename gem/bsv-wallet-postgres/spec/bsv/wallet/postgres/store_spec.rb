...
RSpec.describe BSV::Wallet::Postgres::Store::Postgres do
  ...
  def create_funded_output(satoshis: 1000, vout: 0, basket: nil)
    ...
    output = BSV::Wallet::Postgres::Store::Output.create(action_id: source.id, satoshis: satoshis, vout: vout,
  ...
  describe '#create_action' do
    ...
    it 'creates an action and locks inputs atomically' do
      ...
      expect(BSV::Wallet::Postgres::Store::Input.where(action_id: result[:id]).count).to eq(1)
    end
    ...
    it 'preserves binary input_beef' do
      ...
      action = BSV::Wallet::Postgres::Store::Action[result[:id]]
  ...
  describe '#promote_action' do
    it 'writes outputs, spendable, baskets, details, and tags atomically' do
      ...
      outputs = BSV::Wallet::Postgres::Store::Output.where(action_id: result[:id]).all
      expect(outputs.size).to eq(2)

      # Spendable entries
      expect(BSV::Wallet::Postgres::Store::Spendable.where(output_id: outputs.map(&:id)).count).to eq(2)
    ...
    it 'does not create spendable rows for outbound outputs' do
      ...
      spendable_ids = BSV::Wallet::Postgres::Store::Spendable.where(output_id: outputs.map(&:id)).select_map(:output_id)
  ...
  describe '#link_proof' do
    it 'marks an action as completed' do
      ...
      proof = BSV::Wallet::Postgres::Store::TxProof.first(wtxid: Sequel.blob(wtxid))
  ...
  describe '#abort_action' do
    it 'deletes the action and releases locked inputs via CASCADE' do
      ...
      expect(BSV::Wallet::Postgres::Store::Action[result[:id]]).to be_nil
    ...
    it 'refuses to abort a broadcast action' do
      ...
      BSV::Wallet::Postgres::Store::Broadcast.create(action_id: result[:id])

      store.abort_action(action_id: result[:id])

      # Action should still exist — the broadcast guard prevented deletion
      expect(BSV::Wallet::Postgres::Store::Action[result[:id]]).not_to be_nil
    end
  ...
  describe '#relinquish_output' do
    it 'removes from spendable and basket but keeps the output row' do
      ...
      output = BSV::Wallet::Postgres::Store::Output.where(action_id: action[:id]).first
      ...
      expect(BSV::Wallet::Postgres::Store::Output[output.id]).not_to be_nil
    end
  end
  ...
  describe '#find_or_create_basket' do
    it 'creates a basket and returns its ID' do
      ...
      expect(BSV::Wallet::Postgres::Store::Basket[id].name).to eq('tokens')
    end
  end
  ...
  describe '#resolve_inputs_for_signing' do
    ...
    def create_source_output(wtxid:, satoshis:, vout:, locking_script: nil,
      ...
      locking_script ||= SecureRandom.random_bytes(25)
      source_action = BSV::Wallet::Postgres::Store::Action.create(outgoing: false, description: 'test action',
      ...
      BSV::Wallet::Postgres::Store::Spendable.create(
        output_id: output.id,
        action_id: source_action.id
      )
      output
    ...
    it 'raises when source action has nil wtxid' do
      ...
      BSV::Wallet::Postgres::Store::Spendable.create(output_id: output.id, action_id: source_action.id)
    ...
    it 'rejects corrupt wtxid (hex instead of binary) at database level' do
      ...
      hex_wtxid = 'a' * 64
      expect {
        BSV::Wallet::Postgres::Store::Action.create(outgoing: false, description: 'test action',
  ...
  describe '#reap_stale_actions' do
    it 'deletes stale signed actions with no outputs' do
      ...
      # Backdate the action
      BSV::Wallet::Postgres::Store::Action.where(id: result[:id]).update(created_at: Time.now - 600)
