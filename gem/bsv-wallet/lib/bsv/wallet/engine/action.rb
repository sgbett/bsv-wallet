  ...
  module Wallet
    class Engine
      ...
      class Action
        ...
        # Migrate +Engine#internalize_action+'s body. Parses an incoming
        # Atomic BEEF (BRC-95), runs SPV verification, persists the action
        # as an incoming (broadcast_intent: :none) row, saves ancestor
        # proofs, optionally trims known ancestors to TXID-only, then
        # promotes the caller-named outputs as spendable.
        #
        # @return [Hash] +{ accepted: true }+ — BRC-100 internalizeAction
        #   response shape.
        def self.internalize(engine:, tx:, outputs:, description:, labels: nil,
                             trust_self: nil, known_txids: nil,
                             seek_permission: true, originator: nil)
          # Helper instance: the procedural helpers below live as private
          # instance methods on Action, dispatched through a row-less
          # instance until the action row is created. This mirrors the
          # delegator pattern dev-#284 used for build_outputs et al.
          helper = new(engine: engine, row: { id: nil })

          # Parse tx: as Atomic BEEF (BRC-95)
          beef, subject_tx = helper.send(:parse_beef, tx)
          ...
          helper.send(:hydrate_known_sources!, subject_tx) if trust_self == 'known'
          ...
          helper.send(:verify_incoming_transaction!, subject_tx)
          ...
          engine.store.save_proof(wtxid: subject_tx.wtxid, proof: { raw_tx: subject_tx.to_binary })
          BSV.logger&.debug { "[Engine::Action] internalize: subject=#{subject_tx.dtxid}" }
          ...
          helper.send(:save_beef_proofs, beef, subject_tx.wtxid, action_result[:id])
          ...
          helper.send(:replace_known_ancestors!, beef, subject_tx.wtxid, known_txids) if trust_self == 'known'

          output_specs = outputs.map do |out|
            spec = helper.send(:resolve_internalize_output, out)
          ...
          engine.store.promote_action(action_id: action_result[:id], outputs: output_specs)
        ...
        # @return [Array(String, String, Hash, Transaction::Tx)] wtxid (32-byte
        ...
        def verify_incoming_transaction!(subject_tx)
          ...
          subject_tx.verify(chain_tracker: @engine.chain_tracker)
        rescue BSV::Transaction::VerificationError => e
        ...
        def save_beef_proofs(beef, subject_wtxid, action_id)
          ...
          beef.transactions.each do |beef_tx|
            ...
            proof_id = @engine.store.save_proof(wtxid: wtxid, proof: proof)
          ...
          @engine.store.link_proof(action_id: action_id, tx_proof_id: subject_proof_id) if subject_proof_id
        end
        ...
        def replace_known_ancestors!(beef, subject_wtxid, known_wtxids)
          ...
          beef.transactions.each do |beef_tx|
            ...
            next unless known_set.include?(wtxid) || @engine.store.proof_exists?(wtxid: wtxid)

            BSV.logger&.debug { "[Engine::Action] replace_known_ancestors!: replacing dtxid=#{wtxid.reverse.unpack1('H*')}" }
            beef.make_txid_only(wtxid)
            replaced_count += 1
          end

          BSV.logger&.debug { "[Engine::Action] replace_known_ancestors!: replaced_count=#{replaced_count}" }
