    ...
    module Store
      ...
      class SQLite
        ...
        def promote_action(action_id:, outputs:)
          @db.transaction do
            outputs.map do |out|
              ...
              Spendable.create(output_id: output.id, action_id: action_id) if wallet_owned
        ...
        def query_certificates(certifiers:, types:, limit: 10, offset: 0)
          base = Certificate.where(certifier: certifiers, type: types)
          total = base.count
          records = base.order(Sequel.desc(:created_at)).limit(limit).offset(offset).all
        ...
        def delete_certificate(type:, serial_number:, certifier:)
          Certificate.where(type: type, serial_number: serial_number, certifier: certifier).delete
        end
