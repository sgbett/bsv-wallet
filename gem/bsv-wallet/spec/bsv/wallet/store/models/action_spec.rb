...
RSpec.describe BSV::Wallet::Store::Models::Action, :store do
  ...
  describe 'creation' do
    ...
    it 'preserves an explicit reference' do
      ...
      action = described_class.create(description: 'test action', reference: explicit)
      expect(action.reference).to eq(explicit)
    end

    it 'preserves binary wtxid' do
      wtxid = SecureRandom.random_bytes(32)
      action = described_class.create(description: 'test action', nlocktime: 0, wtxid: wtxid, raw_tx: raw_tx)
    ...
    it 'rejects corrupt wtxid (hex stored as binary) at database level', :postgres do
      ...
      expect do
        described_class.create(description: 'test action', nlocktime: 0, wtxid: Sequel.blob(hex_value), raw_tx: raw_tx)
  ...
  describe 'associations' do
    it 'belongs to tx_proof' do
      action = described_class.create(description: 'test action', nlocktime: 0, wtxid: SecureRandom.random_bytes(32), raw_tx: raw_tx, tx_proof_id: tx_proof.id)
      expect(action.tx_proof).to eq(tx_proof)
    end

    it 'has one broadcast_entry' do
      action = described_class.create(description: 'test action', nlocktime: 0)
    ...
    it 'has many outputs' do
      action = described_class.create(description: 'test action', nlocktime: 0, wtxid: SecureRandom.random_bytes(32), raw_tx: raw_tx)
  ...
  describe '#derived_status' do
    ...
    it 'returns :internal when broadcast_intent is none' do
      action = described_class.create(description: 'test action', nlocktime: 0, wtxid: SecureRandom.random_bytes(32), raw_tx: raw_tx, broadcast_intent: 'none')
