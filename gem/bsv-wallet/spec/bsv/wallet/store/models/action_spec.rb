...
RSpec.describe BSV::Wallet::Store::Models::Action, :store do
  ...
  describe 'creation' do
    ...
    it 'preserves binary wtxid' do
      ...
      action = described_class.create(description: 'test action', wtxid: wtxid, raw_tx: raw_tx)
    ...
    it 'rejects corrupt wtxid (hex stored as binary) at database level', :postgres do
      ...
      expect do
        described_class.create(description: 'test action', wtxid: Sequel.blob(hex_value), raw_tx: raw_tx)
  ...
  describe 'associations' do
    it 'belongs to tx_proof' do
      action = described_class.create(description: 'test action', wtxid: SecureRandom.random_bytes(32), raw_tx: raw_tx, tx_proof_id: tx_proof.id)
  ...
  describe '#derived_status' do
    ...
    it 'returns :internal when broadcast_intent is none' do
      action = described_class.create(description: 'test action', wtxid: SecureRandom.random_bytes(32), raw_tx: raw_tx, broadcast_intent: 'none')
