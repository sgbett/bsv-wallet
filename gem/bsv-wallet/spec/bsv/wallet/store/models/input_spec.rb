...
RSpec.describe BSV::Wallet::Store::Input, :store do
  let(:source_action) { BSV::Wallet::Store::Action.create(outgoing: false, description: 'test action', wtxid: SecureRandom.random_bytes(32), raw_tx: SecureRandom.random_bytes(100)) }
  let(:output) { BSV::Wallet::Store::Output.create(action_id: source_action.id, satoshis: 1000, vout: 0, locking_script: SecureRandom.random_bytes(25), output_type: 'root') }
  let(:spending_action) { BSV::Wallet::Store::Action.create(outgoing: true, description: 'test action', nlocktime: 0) }
  ...
  describe 'structural lock' do
    ...
    it 'prevents double-spend via UNIQUE on output_id' do
      ...
      other_action = BSV::Wallet::Store::Action.create(outgoing: true, description: 'test action', nlocktime: 0)
    ...
    it 'enforces unique vin within an action' do
      output2 = BSV::Wallet::Store::Output.create(action_id: source_action.id, satoshis: 500, vout: 1, locking_script: SecureRandom.random_bytes(25), output_type: 'root')
      described_class.create(action_id: spending_action.id, output_id: output.id, vin: 0)
