...
**Per-method naming register.** This ADR ratified the principle that Engine speaks wallet vocabulary while BRC100 wraps in spec vocabulary, and noted at the time that 26 of the 28 BRC-100 method names matched their Engine primitive (with `Engine#sign_action`, `Engine#encrypt` etc. as the canonical examples). The *current* per-method correspondence — including any spec-name vs wallet-name divergences that emerge as the wallet's primitive surface evolves — is recorded in the living register at [`docs/reference/brc100-conformance.md`](../../../docs/reference/brc100-conformance.md), under the "Engine primitive naming" section. Divergences are added there (not by amending this ADR) because the per-method correspondence is expected to evolve as the wallet's vocabulary matures, while the principle this ADR records is point-in-time and stable. As of the conformance register's first cut, three Engine primitives diverge from the Ruby-idiomatic-snake_case-of-camelCase default (`createAction` → `build_action`, `internalizeAction` → `import_beef`, `listOutputs` → `spendable_outputs`); the register states why each does so, the policy under which future divergences are evaluated, and the Ruby-idiomatic adjustments folded into the default (e.g. predicate convention for `isAuthenticated` → `authenticated?`).
