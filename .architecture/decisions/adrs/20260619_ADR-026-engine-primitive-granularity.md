...
**Stage 2 naming scaffold.** With BRC100 a mixin in Stage 2 (HLR #402 decision 3), 26 of the 28 primitive names share a method name with the BRC-100 spec, so bare invocation from BRC100 method bodies would recurse via MRO. The Stage 2 plan addresses this by prefixing every Engine primitive with `do_` (uniform across all 28). The prefix is scaffolding tied to the mixin constraint; Stage 3's mixin→composition switch reverts it, leaving Engine primitives at the spec-aligned names this ADR's classification ratified (`Engine#sign_action`, `Engine#encrypt`, etc.). The Stage 2 plan (`.claude/plans/20260620-stage-2-primitive-extraction.md`) carries the convention's rationale and the Stage 3 revert commitment.
