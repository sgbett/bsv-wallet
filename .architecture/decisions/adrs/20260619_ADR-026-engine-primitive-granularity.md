...
3. **BRC100 → `engine.<primitive>` only.** No `engine.collaborator.<m>` from the interface layer. Other consumers (e.g. the daemon's broadcast worker, Transmission) may reach collaborators directly when they live in the Engine module's own peer scope — the rule binds *BRC100*, not the wallet at large.
...
6. **Operation preconditions live on the operation.** `require_key_deriver!`, the no_send+deferred-combination check, the sign_and_process+inputs combination check — each raises from inside the primitive that needs the invariant. BRC100 does not pre-check on the primitive's behalf.
...
* **Stage 3's composition discipline becomes structurally enforceable.** "BRC100 only calls public engine primitives" is verifiable by inspection — no `instance_variable_get`, no calls to private methods. Reviewable as a line in the diff.
