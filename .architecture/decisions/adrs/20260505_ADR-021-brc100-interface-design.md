...
* **The schema is canon; the interface is presentation (ADR-003).** BRC-100 is the RPC skin over the canonical database. It orchestrates the components that read and write the schema; it is not itself a source of truth. This is the Layer-3 role in the SOA (`docs/concepts/architecture.md`).
...
* `docs/concepts/architecture.md` — narrative layered shape; BRC-100 is Layer 3 (business-process orchestration).
