...
- **`spendable_intent` denormalisation + composite FK + CHECK** (HLR #467 / [`hot-path-design.md`](hot-path-design.md)) enforces that an output the wallet marked outbound cannot acquire a `spendable` row — declaratively, without a trigger. Replaces the prior `prevent_outbound_spendable` hot-path trigger.
