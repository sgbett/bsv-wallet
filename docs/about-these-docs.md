...
- **`docs/reference/api/`** is the YARD-generated Markdown (mkdocs renders it to HTML), regenerated by `rake docs:generate` (`yardoc --format markdown`). Never edited by hand. It is included in the nav as a single index entry; the per-class pages are reached via that index.
