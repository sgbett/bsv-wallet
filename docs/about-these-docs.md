...
- **`docs/reference/api/`** is the YARD-generated Markdown (Jekyll renders it to HTML), regenerated by `rake docs:generate` (`yardoc --format markdown`). Never edited by hand. It is excluded from the side nav and search index via the `defaults:` block in `_config.yml`; the per-class pages are reached via the API Reference entry under Reference.
