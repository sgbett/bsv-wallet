...
Both gates take a cross-table rule a single-row CHECK cannot reach and reformulate it so the schema *can* enforce it declaratively. The principle is: enforcement stays in the database (ADR-003), and the mechanism is chosen with the scale target in mind. Hot-path triggers remain legitimate where no declarative form is available and the path can afford the per-row cost (the wallet has two such triggers: `prevent_outbound_spendable` and `prevent_internal_action_delete`).
