...
| `task.succeeded` | `task` `id` `latency_ms` `outcome` | Work completed successfully. For `broadcast_push`, this is also the trigger that fires Phase 4: when ARC returns an accepted status, `Engine::Broadcast` invokes `Store#promote_action_outputs` to flip the action's outputs from `promoted = false` to `true` and insert their spendable rows. |
