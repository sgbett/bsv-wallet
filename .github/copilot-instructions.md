...
2. **Consult the [`reference/`](../reference) doc that covers the subsystem the PR touches** — read the relevant one rather than guessing:
   ...
   - [`action-lifecycle.md`](../reference/action-lifecycle.md) — the action state machine: the 4-phase send path, the internal path, and the per-transition validation matrix.
